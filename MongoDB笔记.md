...
#### GridFS命令行工具mongofiles
> mongofiles options commands filename

上传文件

> mongofiles -d mytest -l spring-boot-reference-2.0.2.pdf put spring-boot-reference.pdf
...
列出所有文件
> mongofiles -d mytest list

![](img/p122.png)


列出以sp开头的所有文件(sp 不能加引号)
> mongofiles -d mytest list sp

![](img/p123.png)

查询文件名包含给定字符串的文件

> mongofiles -d mytest search s

![](img/p124.png)

下载文件

> mongofiles -d mytest -l /Users/wujinqing/Downloads/hello.pdf get spring-boot-reference.pdf

![](img/p125.png)

删除文件
> mongofiles -d mytest delete spring-boot-reference.pdf

![](img/p126.png)


### MongoDB Capped Collection 固定大小的集合
> Capped Collection是一个固定大小的集合，支持高吞吐量操作比如插入和获取。

1. 没有索引，减少了维护索引的开销，提高了插入操作的吞吐量。
2. 插入顺序是按磁盘顺序。
3. 自动删除老的文档。
4. 不支持分片。
...
创建一个固定大小100个字节,且最多插入两条数据的集合， size是必须的，优先级高。

> db.createCollection("log", {capped: true, size: 100, max:2})
![](img/p127.png)

检查一个集合是否为Capped Collection
> db.log.isCapped()

将一个集合转换成Capped Collection
