...
|---|---|
|--dbpath|指定数据文件存储路径|
|--fork|以后台进程的方式启动mongod|
|--logpath|指定日志文件存储路径|
|--logappend|指定日志是以追加的方式添加，而不是覆盖的方式|
|--port|指定服务端口号|
|--bind_ip|绑定服务IP|
|--keyFile|指定key文件存放路径|
|--auth|指定需要安全验证即需要用户名密码才能访问|
|--replSet|指定副本集名称|
|--oplogSize|指定副本集日志文件大小|
|--directoryperdb|每个数据库都存放在一个单独的文件夹下|




Last login: Sun Jul 22 11:41:07 on ttys003
➜  ~ mongod -h
Options:
